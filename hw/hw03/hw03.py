...
def digit_distance(num):
    ...
    if num < 10:
        ...
        return digit_distance(num // 10) + abs(num % 100 // 10 - num % 10)
...
def interleaved_sum(num, f_odd, f_even):
    ...
    True
    """
    def inner(k, f_current, f_next):
        if k > num:
            return 0 
        return f_current(k) + inner(k + 1, f_next, f_current)
    return inner(1, f_odd, f_even)
...
def count_dollars(sum_needed):
    ...
    def count(sum, bill):
        ...
        if sum < 0:
            return 0
        if bill:  
            return count(sum, next_smaller_dollar(bill)) + count(sum - bill, bill)
        else:
            return 0
...
def count_dollars_upward(sum_needed):
    ...
    True
    """
    def count_upward(sum, bill):
        if sum == 0:
            return 1
        if sum < 0:
            return 0
        if bill:
            return count_upward(sum - bill, bill) + count_upward(sum, next_larger_dollar(bill))
        else:
            return 0
    return count_upward(sum_needed, 1)
